...
export class DatabaseBuilder {
  ...
  private makeFinalChannel(channel: PChannel): Channel {
    ...
    return {
      ...
      participants:
        participants.length > 0 ? participants.map((oldAuthorIndex) => this.authorsRank[oldAuthorIndex]) : undefined,
  ...
  build(): Database {
    if (this.numMessages === 0) throw new Error('No messages found. Are you sure you are using the right platform?')
