...
const fn: BlockFn<InteractionStats> = (database, filters, common, args) => {
  ...
  const processMessage = (msg: MessageView) => {
    ...
    if (reactions) {
      ...
      if (reactionCount > 0) {
        if (topTotalReactions.length < 3 || reactionCount > topTotalReactions[topTotalReactions.length - 1][1]) {
      ...
      if (maxReactionCount > 0) {
        if (topSingleReactions.length < 3 || maxReactionCount > topSingleReactions[topSingleReactions.length - 1][1]) {
